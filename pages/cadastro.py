...
def show_cadastro():
    ...
    if "cadastro_realizado" in st.session_state and st.session_state.cadastro_realizado:
        ...
        ultimo_usuario = st.session_state.usuarios[-1] if st.session_state.usuarios else {}
        st.info(f"""
        ...
        **Dados da conta criada:**
        - **Nome:** {ultimo_usuario.get('nome_completo', '')}
        - **Usuário:** {ultimo_usuario.get('usuario', '')}
        - **E-mail:** {ultimo_usuario.get('email', '')}
        - **Instituição:** {ultimo_usuario.get('instituicao', '')}
        """)
        ...
        with col1:
            if st.button("Ir para Login", type="primary", use_container_width=True):
                ...
                st.rerun()
        with col2:
            if st.button("Cadastrar Outro Usuário", use_container_width=True):
                st.session_state.cadastro_realizado = False
                st.rerun()
    else:
        ...
        with st.form("cadastro_form", clear_on_submit=True):
            ...
            if submitted:
                ...
                if erros:
                    ...
                    # Marcar que o cadastro foi realizado
